...
function Checkout() {
    ...
    for (i = 0; i < shoppingCartArray.length; i++) {   //for each item in the array:
            var elmnt = document.getElementsById("item");
            var cln = elmnt.cloneNode(true);
            cln.onclick = function(i){
                shoppingCartArray.splice(i, 1)
                location.reload();
            }
            document.body.appendChild(cln);
        //a = document.importNode(glaze, true); //Create a new node, based on the template:
        //console.log("shopa[i] is: " + shoppingCartArray[i]);
        //a.textContent += shoppingCartArray[i].glaze + " " + shoppingCartArray[i].amount; //Add data from the array:
